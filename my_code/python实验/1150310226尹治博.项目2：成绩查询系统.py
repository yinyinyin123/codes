...
from  Tkinter  import  *
import  sqlite3
aa=sqlite3.connect('C:\\Users\\yinyin\\Desktop\\cu')#创建数据库
bb=aa.cursor()
...
def   wo0():
    def  haha():
        ...
        aa.commit()
        bb.execute("select  kaohao from  catalog")#将考号从系统中抽出
        list1=bb.fetchall()
        print  list1
    subroot=Tk()
    ...
    subroot.mainloop()
def  wo1():
    bb.execute("select  kaohao  from  catalog")#选考号
    list0=bb.fetchall()
    bb.execute("select  xingming  from  catalog")#选姓名
    list7=bb.fetchall()
    bb.execute("select  zongchengji  from  catalog")#总成绩
    list8=bb.fetchall()
    dic={}   #建立字典
    list6=[]
    list9=[]
